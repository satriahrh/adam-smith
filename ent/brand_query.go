...
func (bq *BrandQuery) sqlAll(ctx context.Context) ([]*Brand, error) {
	...
	if query := bq.withProducts; query != nil {
		...
		ids := make(map[int]*Brand, len(nodes))
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(brand.ProductsPrimaryKey[0], fks...))
			},
...
func (bq *BrandQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if ps := bq.order; len(ps) > 0 {
		_spec.Order = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector, brand.ValidColumn)
			}
		}
	}
	return _spec
...
func (bgb *BrandGroupBy) sqlQuery() *sql.Selector {
	...
	for _, fn := range bgb.fns {
		columns = append(columns, fn(selector, brand.ValidColumn))
	}
